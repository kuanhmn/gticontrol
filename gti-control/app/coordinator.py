...
class Coordinator:
    ...
    def build_state(self, device_id: str) -> Dict:
        ...
        if self.server_enabled and self.use_server_daily_monthly and not self.expose_totals_only:
            ...
            for dk in ["energy_daily","grid_energy_daily","tieuthu_energy_daily"]:
                st[dk] = fmt2(srv.get(dk, st.get(dk, 0.0)))
            for mk in ["energy_monthly","grid_energy_monthly","tieuthu_energy_monthly"]:
                st[mk] = fmt2(srv.get(mk, st.get(mk, 0.0)))
        return st

    def publish_state(self, device_id: str, st: Dict):
        self.client.publish(f"gti/{device_id}/state", json.dumps(st), retain=True)
